...
class PromiseService:
    ...
    NEITHER_PARTY_NOR_UNION = _("Neither party nor union is set!")
    BOTH_PARTY_AND_UNION = _("Both party and union are set!")
    PARTY_NOT_FOUND = _("Political party does not exist!")
    UNION_NOT_FOUND = _("Political union does not exist!")
    ...
    def _ensure_party_xor_union(self, party_id: UUID | None, union_id: UUID | None) -> None:
        if party_id is None and union_id is None:
            raise ApplicationError(self.NEITHER_PARTY_NOR_UNION)

        if party_id and union_id:
            raise ApplicationError(self.BOTH_PARTY_AND_UNION)

    def _load_party_or_union(self, party_id: UUID | None, union_id: UUID | None):
        party = None
        union = None
    ...
    def _ensure_elected_in_convocation(
        self, convocation: Convocation, party: PoliticalParty | None, union: PoliticalUnion | None
    ) -> None:
        ...
        if union is not None:
            if not convocation.political_unions.filter(id=union.id).exists():
                raise ApplicationError(
                    self.PARTY_NOT_ELECTED_IN_CONVOCATION.format(name=union.name, convocation=convocation.name)
                )
