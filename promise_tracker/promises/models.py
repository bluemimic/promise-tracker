...
class Promise(BaseModel):
    ...
    union: Field = models.ForeignKey(
        ...
        help_text=_("The political union associated with this promise."),
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            CheckConstraint(
                check=Q(party__isnull=True) | Q(union__isnull=True),
                name="promise_party_xor_union",
                violation_error_message=_("Both party and union are set."),
            ),
            CheckConstraint(
...
class Comment(BaseModel, SoftDeleteModel):
    ...
    promise: Field = models.ForeignKey(
        ...
        verbose_name=_("Promise"),
        help_text=_("The promise associated with this comment."),
    )
    response_to: Field = models.ForeignKey(
        to="self",
        null=True,
        blank=True,
        on_delete=models.PROTECT,
        ...
        help_text=_("The comment this comment is responding to."),
    )

    def __str__(self) -> str:
        return f"Comment on {self.promise.name}"
