...
class PromiseFilterSet(FilterSet):
    ...
    union = ModelChoiceFilter(
        field_name="union__id",
        queryset=PoliticalUnion.objects.all(),
        label=_("Union"),
        help_text=_("Filter by union"),
        method="filter_union",
    )
    ...
    def filter_union(self, queryset: QuerySet[Promise], name: str, value: PoliticalUnion) -> QuerySet[Promise]:
        if value:
            return queryset.filter(union=value)
        return queryset
